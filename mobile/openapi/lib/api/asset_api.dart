...
class AssetApi {
  ...
  /// Performs an HTTP 'PUT /asset/{id}/isfavourite' operation and returns the [Response].
  /// Parameters:
  ///
  /// * [String] id (required):
  ///
  ...
  ///
  /// * [String] key:
  Future<Response> updateIsFavouriteWithHttpInfo(String id, UpdateAssetDto updateAssetDto, { String? key, }) async {
    // ignore: prefer_const_declarations
    final path = r'/asset/{id}/isfavourite'
      .replaceAll('{id}', id);
  ...
  Future<AssetResponseDto?> updateIsFavourite(String id, UpdateAssetDto updateAssetDto, { String? key, }) async {
    final response = await updateIsFavouriteWithHttpInfo(id, updateAssetDto,  key: key, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
